...
import { updateLeadAction } from "../../../../service/apis/adminInquiry.actions";
...
export const useInquiryDetails = (candidate: CandidateLead) => {
  ...
  const notifPrefs = candidate.notificationPreference || {
    email: false,
    sms: false,
    whatsapp: false,
  };
  ...
  const fh = (field: string) =>
    inquiryForm.touched[field as keyof typeof inquiryForm.touched]
      ? (inquiryForm.errors[
          field as keyof typeof inquiryForm.errors
        ] as string)
      : undefined;
