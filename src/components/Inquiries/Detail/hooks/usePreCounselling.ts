...
import { updateAssignmentAction } from "../../../../service/apis/adminInquiry.actions";
import { confirmToast } from "../../../../utils/confirmToast";
import { CamelCase, isWithinSchedule } from "../../../../utils/common";
import type { AssignmentStatus, IAssignment } from "../../../../types/models.types";
...
export const usePreCounselling = (
  ...
  const preForm = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      ...
      if (
        values.preStatus === "completed" ||
        values.preStatus === "rejected"
      ) {
        const actionText =
          values.preStatus === "completed" ? "Completed" : "Rejected";
        const confirmed = await confirmToast(
          `Are you sure Candidate is ${actionText}!`
        );
        if (!confirmed) {
          setSubmitting(false);
          return;
      ...
      try {
        ...
        if (
          preResult?.data?.data?.status === "completed" ||
          preResult?.data?.data?.status === "rejected"
        ) {
          setIsPreLocked(true);
        }
  ...
  const updateAssignmentStatus = async (status: string) => {
    ...
    try {
      ...
      preForm.setValues({
        ...preForm.values,
        preStatus: status as AssignmentStatus,
      });
