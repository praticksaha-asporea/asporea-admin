...
import type { AssessBasicFormValues, ExpStatus, ExpType } from "../../../../types/object.types";
import type { CandidateLead } from "../../../../types/payloads/Candidate.types";
import type { IAssignment } from "../../../../types/models.types";

export const useAssessmentFormSection = (
  ...
  const [expType, setExpType] = useState<ExpType>(
    candidate?.experience?.type as ExpType
  );
  ...
  const assessBasicForm = useFormik<AssessBasicFormValues>({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      if (!assessAssign?._id) {
        toast.error("No assessment assignment found");
        setSubmitting(false);
        return;
      }
      if (values.status === "completed") {
        const confirmed = await confirmToast(
          `Are you sure Pre-Counselling is Completed!`
        );
        if (!confirmed) {
          setSubmitting(false);
          return;
      ...
      if (values.status === "rejected") {
        const confirmed = await confirmToast(
          `Are you sure Candidate is Rejected!`
        );
        if (!confirmed) {
          setSubmitting(false);
          return;
      ...
      try {
        ...
        if (
          assessResult?.data?.data?.status === "completed" ||
          assessResult?.data?.data?.status === "rejected"
        ) {
          setIsPreLocked(true);
        }
 ...
 console.error(err)      } finally {
    ...
    if (candidate?.status === "doc_awaiting_approval") {
      ...
      setShowAssessmentForm(
        docs?.status === "verified" &&
          candidate?.experience?.status === "verified"
      );
    }
  ...
  }, [assessAssign, candidate, docs, tech]);
  ...
  const updateAssignmentStatus = async (status: string) => {
    ...
    try {
      ...
      assessBasicForm.setValues({
        ...assessBasicForm.values,
        status: status,
      });
  ...
  const handleSaveAll = async () => {
    if (assessBasicForm.values.status === "rejected") {
      ...
      await updateAssignmentAssessAction(formData);
      toast.success(
        `Status updated to ${CamelCase(assessBasicForm.values.status)}`
      );
      assessBasicForm.setValues({
        ...assessBasicForm.values,
        status: assessBasicForm.values.status,
      });
      setIsPreLocked(true);
    }
  };
  ...
  const updateExpStatus = async (
    ...
    if (!confirmed) return;
    try {
      const updatedEXPLead = await updateExpStatusAction({
        id: assessAssign._id.toString(),
        status,
        expType,
      });
      ...
      if (
        updatedEXPLead?.data?.data?.experience?.status === "verified"
      ) {
      ...
      setExpType(
        updatedEXPLead?.data?.data?.experience?.type as ExpType
      );
    } catch (err: any) {
 console.error(err)    }
  };
  ...
  const canMarkNotResponded =
    canAccess && !isFinalStatus && currentStatus === "contacted";
