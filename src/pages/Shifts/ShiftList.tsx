...
import { useShiftList, PAGE_SIZE } from "./useShiftList";
import type { ShiftResponseData } from "../../types/responses/shift/shift.responses";
import type { ScheduleObj } from "../../types/payloads/shift/shift.payloads";
function ShiftNameCell({
  shiftName,
  schedules,
}: {
  shiftName: string;
  ...
  return (
    <div className="space-y-1.5">
      {schedules.map((sch, i) => (
        <div
          key={i}
          className="flex items-center gap-2 bg-gray-50 px-3 py-1.5 rounded-xl w-fit"
        >
          <Clock className="w-3.5 h-3.5 text-gray-400 shrink-0" />
          ...
          <span className="text-[10px] font-bold text-[#fc7728] bg-orange-50 px-2 py-0.5 rounded-md">
            {sch.days.slice(0, 3).join(", ")}
            {sch.days.length > 3 ? ` +${sch.days.length - 3}` : ""}
          </span>
        </div>
      ))}
    </div>
  ...
  return (
    <div className="flex flex-wrap gap-1">
      {breaks.map((b) => (
        <span
          key={b}
          className="text-xs font-bold text-[#0D80F2] bg-blue-50 px-2 py-1 rounded-md"
        >
          {b}
        </span>
      ))}
...
const columns: ColumnDef<ShiftResponseData>[] = [
  {
    header: "Shift",
    accessor: (row) => (
      <ShiftNameCell shiftName={row.shiftName} schedules={row.schedules} />
    ),
  },
  {
    header: "Schedules",
