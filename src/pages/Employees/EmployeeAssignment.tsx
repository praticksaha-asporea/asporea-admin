...
import { useEmployeeAssignment, PAGE_SIZE } from "./useEmployeeAssignment";
import type { AssignmentResponseData } from "../../types/responses/assignment/assignment.responses";
...
function LocationShiftCell({ assignment }: { assignment: AssignmentResponseData }) {
  const branchName =
    typeof assignment.branchId === "object"
      ? assignment.branchId.title
      : assignment.branchId;
  const shiftName =
    typeof assignment.shiftId === "object"
      ? assignment.shiftId.shiftName
      : assignment.shiftId;
  return (
    <div className="space-y-1">
      <p className="text-sm font-bold text-gray-700 flex items-center gap-1.5">
...
const columns: ColumnDef<AssignmentResponseData>[] = [
  ...
  },
  {
    ...
    accessor: (row) => (
      <ConfigCell
        minuteOfSlots={row.minuteOfSlots}
