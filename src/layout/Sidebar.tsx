...
const Sidebar = ({ isOpen, setIsOpen }: SidebarProps) => {
  ...
  useEffect(() => {
    if (location.pathname.includes("/branches") || location.pathname.includes("/shifts")) {
  ...
  return (
    <aside
      className={`bg-white border-r border-gray-100 flex flex-col transition-all duration-300 ease-in-out z-20 ${isOpen ? "w-64" : "w-20"
        }`}
    >
      <div className="h-20 flex items-center justify-center border-b border-gray-50">
        <img
          src={logo}
          alt="Asporea Logo"
          className={`transition-all mt-4 duration-300 object-contain ${isOpen ? "w-48" : "w-14"
            }`}
        />
      </div>
        ...
        {menuItems.map((item) => {
              ...
              <div key={item.name} className="flex flex-col">
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <span
                      className={`whitespace-nowrap transition-all duration-300 ${isOpen ? "opacity-100 block" : "opacity-0 hidden"
                        }`}
                    >
                      {item.name}
                    </span>
          ...
          return (
            <NavLink
              ...
              to={item.path}
              className={({ isActive }) =>
                `flex items-center gap-4 px-3 py-3 rounded-xl transition-all font-medium ${isActive
